 ...
 * 
 */
public class RandomPlusKIAlgorithm implements IKIAlgorithm {
	...
	while (!playerPos.isEmpty() && !found) {
	    ...
	    // 3. Zuege berechnen
	    playerMoves = StaticKIMethods.findMoves(board, workingPos);
	    System.err.println(playerMoves);
	    // 4. Zug waehlen oder zurueck zu 2.
	    if (!playerMoves.isEmpty()) {
		move = chooseGoodMove(board, playerList, activePlayer,
			playerMoves, random);
	...
	for (ArrayList<BoardPosition> moveList : playerMoves) {
	    ...
	    value = StaticKIMethods.evaluateGameSituation(board, playerList,
		    activePlayer);
	    System.err.println("RandomPlus value: " + value);
	    if (maxValue < value) {
		goodMoves.clear();
