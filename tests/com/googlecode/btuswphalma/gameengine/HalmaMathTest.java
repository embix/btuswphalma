...
public class HalmaMathTest {
    ...
    /**
     * Fuer alle Moeglichen Werte, die ein Byte haben kann, wird die Methode
     * getestet (by byte noch Sinnvoll)
     * 
     * Test method for
	...
	for (int x = 0; x < correctBoardAnswers.length; x++) {
		...
		Assert.assertEquals(correctBoardAnswers[x][y], HalmaMath
			.isOnBoard((byte) x, (byte) y));
	    }
	}
    ...
    /**
     * Der Test wird noch ausgelassen, ist aber nicht sehr spannend Test method
     * for
    ...
    /**
     * Fuer alle Boardpositionen als werden alle moeglichen Ziele getestet. Der
     * Test ist Fraglich, da die Formel zum ermitteln der richtigen Antwort wohl
     * der selbe wie in der getesteten Methode ist. Zwat wurde sie nicht
     * uebernommen, aber von der selben Person erstellt. Test method for
	...
	// Fuer jede potentielle Spielbrettposition (innerhalb des 17*13
	// Gebiets) werden alle moeglichen
	...
	for (byte x = -128; x < 127; x++) {
		...
		for (byte xs = 0; xs <= 16; xs++) {
			...
			Assert.assertEquals(res, HalmaMath.isPushDistance(
				startPos, endPos));
			Assert.assertEquals(false, HalmaMath.isPushDistance(
				startPos, null));
			Assert.assertEquals(false, HalmaMath.isPushDistance(
				null, endPos));
			Assert.assertEquals(false, HalmaMath.isPushDistance(
	...
	for (byte x = -128; x < 127; x++) {
		...
		for (byte xs = 0; xs <= 16; xs++) {
			...
			Assert.assertEquals(res, HalmaMath.isJumpDistance(
				startPos, endPos));
			Assert.assertEquals(false, HalmaMath.isJumpDistance(
				startPos, null));
			Assert.assertEquals(false, HalmaMath.isJumpDistance(
				null, endPos));
			Assert.assertEquals(false, HalmaMath.isJumpDistance(
